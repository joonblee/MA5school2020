...
void higgsMassCombined() {
  ...
  for( unsigned i=0; i<File.size(); i++ ) {
    Hist.push_back( (TH1D*)File[i]->Get("M_mumubb") );
    Hist[i]->SetTitle("");
  ...
  Hist[0]->GetXaxis()->SetTitle("m_{#mu#mubb} (GeV)");
  Hist[0]->GetYaxis()->SetTitle("Events / 5 GeV");
  Hist[0]->GetYaxis()->SetRangeUser(0.1,4.);
  Hist[0]->GetYaxis()->SetMoreLogLabels();
  Hist[0]->GetYaxis()->SetNoExponent();
  ...
  Canvas->SaveAs("../plots/higgsMassCombined.png");
  Canvas->SaveAs("../plots/higgsMassCombined.pdf");
}
